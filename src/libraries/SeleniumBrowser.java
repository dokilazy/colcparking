...
public class SeleniumBrowser {
	 ...
	 *            Text to select
	 */
	public String selectRandomDropdown(By pBy) {
		Select mSelect = new Select(this.Driver.findElement(pBy));
	...
	//
	// public void getTableData(By pTable) {
	// WebElement Webtable = this.captureInterface(pTable);
	// List<WebElement>
	// TotalRowCount=Webtable.findElements(By.xpath("//*[@id='TableID']/tbody/tr"));
	//
	...
	// records.put("s3", "c3", 6);
	//
	// WebElement table_element = this.captureInterface(pTable);
	// List<WebElement> tr_collection = table_element.findElements(By.xpath(pTable +
	// "/tbody/tr"));
	//
	...
	public List findRowElementsEditInline(String SearchValue) {
		try {
			WebElement row = this.Driver.findElement(By.xpath(
					"//table[contains(@id,'DataTables_Table')]/tbody/tr/td/form/input[contains(@attr-value,'" + SearchValue + "')]/../../.."));
			int NumOfCol = row.findElements(By.xpath("td")).size();
	...
	public void takeScreenshot(String className, String method, LocalTime timestamp) {
		if (this.Driver instanceof TakesScreenshot) {
			...
			try {
				File localScreenshots = new File(new File("src/Execution Reports"), "Failed Case Screenshots");
