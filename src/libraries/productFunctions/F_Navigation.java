...
public class F_Navigation {
	...
	/**
	 * Login to ANPR Global Site with default user
	 */
	public static void loginGlobal(SeleniumBrowser Browser, String tUser, String tPass) {
		try {
			Browser.Driver.navigate().to(Constants.globalUrl);
	...
	/**
	 * Login to ANPR Local Site 
	 * with default user
	 */
	public static void loginLocal(SeleniumBrowser Browser, String tUser, String tPass) {
		try {
			Browser.Driver.navigate().to(Constants.localUrl);
		} catch (TimeoutException e) {
			try {
				Browser.Wait.until(ExpectedConditions.elementToBeClickable(I_Global_Common.txt_Username));
			} catch (Exception e2) {
