...
public class F_CPVerification {
	...
	public static void verifyVRMexistinTable(SeleniumBrowser Browser, String vrm, String cam, String eDate,
			TCResult Result) {
		F_CPSettings.commonSearch(Browser, vrm);
		List<String> firstRow = Functions.getFirstRow(Browser);
		F_GeneralVerification.verifyElementValue(Browser, "Vehicle Number", firstRow.get(0), vrm, Result);
		F_GeneralVerification.verifyElementContent(Browser, "Camera Name", firstRow.get(1).replace(" ", ""),
				cam.replace(" ", ""), Result);
		F_GeneralVerification.verifyElementContent(Browser, "Exception Date", firstRow.get(2), eDate, Result);
	}
	...
	public static void verifyVRexistinTable(SeleniumBrowser Browser, String voidReason, String required,
		...
		List<String> firstRow = Functions.getFirstRow(Browser);
		F_GeneralVerification.verifyElementValue(Browser, "void Reason Name", firstRow.get(0), voidReason, Result);
	...
	public static void verifyOffenceExistinTable(SeleniumBrowser Browser, String type, String code, TCResult Result) {
		...
		List<String> firstRow = Functions.getFirstRow(Browser);
		F_GeneralVerification.verifyElementValue(Browser, "Offence type", firstRow.get(0), type, Result);
	...
	public static void verifyImportJson(SeleniumBrowser Browser, String exceptionDate, String cameraEncoderNumber,
		...
		for (int n = 0; n < vrmNumber.size(); n++) {
			F_CPVerification.verifyVRMexistinTable(Browser, vrmNumber.get(n).toString(),
					F_CP_common.Camera(cameraEncoderNumber), exceptionDate, pResult);
		}
	}

	public static void verifyImportExcel(SeleniumBrowser Browser, List exceptionDate, List cameraEncoderNumber,
			List vrmNumber, TCResult pResult) {
		for (int n = 0; n < vrmNumber.size(); n++) {
			String date = Functions.changeDateFormat(exceptionDate.get(n).toString(), CPValueList.dateFormat.formatUS);
			F_CPVerification.verifyVRMexistinTable(Browser, vrmNumber.get(n).toString(),
					F_CP_common.Camera(cameraEncoderNumber.get(n).toString()), date, pResult);
		}
	}

	public static void verifyDownloadedFile(SeleniumBrowser Browser, O_contravention conv, TCResult pResult) {
