...
public class CPSettings extends DefaultAnnotations {
	...
	public void Verify_add_new_VRM() {
		...
		String eDate = Functions.getCurrentDate(CPValueList.dateFormat.formatUS); // Functions.getCurrentDateinformatMMddyyyy();
		String cam = "";
		...
		F_CPSettings.inputExceptionDate(Browser, eDate);
		cam = F_CPSettings.selectRandomCamera(Browser);
		...
		F_CPVerification.verifyVRMexistinTable(Browser, vrm, cam, eDate, Result);
	...
	public void Verify_edit_VRM() {
		...
		String cam = "";
		String eDate = Functions.getCurrentDate(CPValueList.dateFormat.formatUS);
		...
		F_CPSettings.goToPage.VRM(Browser);
		F_CPVerification.verifyVRMexistinTable(Browser, newVrm, cam, eDate, Result);
	...
	public void Verify_VRM_import_by_Json() {
		...
		F_CPVerification.verifyImportJson(Browser, eDate, cameraEncoderNumber, vrmNumber, Result);
		Assert.assertTrue(Result.Result, Result.Message);
	...
	public void Verify_VRM_import_by_Excel() {
		...
		List exceptionDate =  F_CP_common.createExceptionDateList(s);
		F_CP_common.createVRMExcel(exceptionDate, cameraEncoderNumber, vrmNumber);
		...
		Functions.uploadfile(Constants.DataPath, "VRM.xlsx");

		F_GeneralVerification.verifyNoticationMessage(Browser, I_CP_Common.div_SuccessMessage,
				MessageFormat.format(CPValueList.Message.CreateSuccess, "VRM Exception"), Result);
		
		F_CPVerification.verifyImportExcel(Browser, exceptionDate, cameraEncoderNumber, vrmNumber, Result);
